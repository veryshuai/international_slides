...
\begin{figure}
	\centering
		\includegraphics[scale=0.25]{fiscal_expansion_perm.png}
	\label{fig:11}
\end{figure}
}
...
\begin{itemize}
\item XX curve: combinations of output and exchange rates at which the current account is at some desired level.
\begin{itemize}
    \item if $Y \Uparrow \Rightarrow CA \Downarrow$ as imports rise 
    \item $E$ must rise to maintain desired level of current account
\end{itemize}
\item XX curve slopes upward but is flatter than the DD curve
\begin{itemize}
    \item As output increases, consumption only absorbs part 
    \item The rest needs to be absorbed by current account
    \item In other words, $E$ must rise enough so current account can increase! 
    \item $XX$ curve: $E$ rises enough so that current account is constant
\end{itemize}
\end{itemize}
}

\frame[plain]{
\frametitle{How Macroeconomic Policies Affect the Current Account}
\begin{figure}
	\centering
		\includegraphics[scale=0.25]{current_account.png}
	\label{fig:11}
\end{figure}
}
...
\frame[plain]{
\frametitle{The J-Curve}
\begin{itemize}
    \item Current account adjustment follows a J-shaped curve 
\end{itemize}
\begin{figure}
	\centering
		\includegraphics[scale=0.25]{j_curve.png}
\end{figure}
}

\frame[plain]{
\frametitle{Pass-through}
\begin{itemize}
    \item Imperfect competition
    \begin{itemize}
        \item Firms make money
        \item When costs rise, who pays?
        \item The amount of a cost rise passed-on to consumers is called pass-through
    \end{itemize}
    \item Exchange rate pass-through
    \begin{itemize}
        \item Are exchange rates changes passed on to consumers?
        \item Literature still developing, but usually find almost nothing!
        \item Means currency movements will not be reflected in current account
    \end{itemize}
\end{itemize}
}
...
\begin{frame}{Liquidity trap}
    \begin{itemize}
        \item Suppose interest rates on illiquid assets fall to zero (draw it)
        \item Increasing money supply cannot change the exchange rate (calculate it)
        \item Going to make a flat $AA$ curve as in the next picture 
    \end{itemize}
\end{frame}

\begin{frame}{Liquidity trap}
    \includegraphics[scale=0.25]{liquidity_trap.png}
\end{frame}
